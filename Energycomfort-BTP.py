...
def marge_format_excel(names_files: list) -> Workbook:
    ...
    # Load basic Excel
    wb_basic = load_workbook(filename=r'Z:\Excel SW\ТЗ' + f'\{names_files[0]}')
    ws = wb_basic.active
...
def edit_add_frame(names: list, sort_data: list, title: list) -> list:
    ...
    # Load frame Excel
    wb_frame = load_workbook(filename=r'Z:\Excel SW\ТЗ' + f'\{names}')
...
name_files = ''
for root, dirs, files in os.walk(r"Z:\Excel SW\ТЗ"):
    name_files = files
...
for name_files in group_names:
    ...
    wb.save(f'Z:\Excel SW\{name_files[0].split(".")[0]} Итог.xls')
